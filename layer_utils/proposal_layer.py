...
def proposal_layer_tf(rpn_cls_prob, rpn_bbox_pred, im_info, cfg_key, _feat_stride, anchors, num_anchors):
  ...
  # Get the scores and bounding boxes
  scores = rpn_cls_prob[:, :, :, num_anchors:]
  scores = tf.reshape(scores, shape=(-1,))  # [num_anchors]
  rpn_bbox_pred = tf.reshape(rpn_bbox_pred, shape=(-1, 4))  # [num_anchors, 4]

  proposals = bbox_transform_inv_tf(anchors, rpn_bbox_pred)  # [num_anchors, 4]
